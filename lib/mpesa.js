...
class MpesaAPI {
  constructor() {
    ...
    // Your actual M-Pesa API endpoint - replace with your actual API URL
    this.baseURL = 'https://api.pesapath.com/v1'; // Your actual M-Pesa API
  }
  ...
  async stkPush(phone, amount, reference = 'Test Payment') {
    try {
      ...
      return {
        success: true,
        data: response.data,
        transactionId: response.data.transaction_request_id || response.data.TransactionId
      };
    } catch (error) {
